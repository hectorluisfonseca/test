...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      selectInput(inputId = "subset_income", label = "Choose income category", 
                  choices = unique(income$income)),
      # Interactive piece 2: inputId = "set_yaxis"
      selectInput(inputId = "set_yaxis", label = "Choose y-axis variable", 
                 choices = c(hours_per_week = "hours_per_week",capital_loss = "capital_loss" )),
      # Interactive piece 3: inputId = "subset_occupation"
    checkboxGroupInput(inputId = "subset_occupation", label = "Choose occupations", 
